    ...
    restart:
        ...
        case FS_Type: {
            ...
            integer:
                ...
                if (precision >= 0) {
                    precision = std::min(precision, (int32_t)countof(work8));
                }
                else {
                    precision = 1;
                }
                ...
                char* end = &work8[countof(work8) - 1];
                char* start = end;
                start[0] = '\0';
            ...
            case 'n': {
                ...
                if (flags & FF_IsShort) {
                    ...
                    *(uint16_t*)(g_memory.Translate(pointer)) = be<uint16_t>((uint16_t)count).value;
                    //SHIM_SET_MEM_16(pointer, (uint16_t)count);
                }
                else {
                    *(uint32_t*)(g_memory.Translate(pointer)) = be<uint32_t>((uint32_t)count).value;
        ...
        if (!text.is_wide) {
            ...
            if (text.swap_wide) {
                while (remaining-- > 0) {
                    if (!data.put((uint16_t)be<uint16_t>(*b++).value)) { // BYTESWAP
                        return -1;
                    }
                }
            }
...
PPC_FUNC(__imp__vswprintf)
{
    ...
    uint16_t* buffer = (uint16_t*)g_memory.Translate(buffer_ptr);
    uint16_t* format = (uint16_t*)g_memory.Translate(format_ptr);
